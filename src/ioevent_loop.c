...
static void deal_ioevents(IOEventPoller *ioevent)		// 处理触发的事件
{
	int event;
	IOEventEntry *pEntry;

	for (ioevent->iterator.index=0; ioevent->iterator.index < ioevent->
            iterator.count; ioevent->iterator.index++)		// 事件循环处理s
	{
        ...
        if (pEntry != NULL) {
            pEntry->callback(pEntry->fd, event, pEntry);		// 触发对应的事件回调
        }
        else {
	...
	clean_up_callback, volatile bool *continue_flag)
{
	...
	while (*continue_flag)
	{
		...
		{
			deal_ioevents(&pThreadData->ev_puller);		// 真正有数据来进入该函数
		}
